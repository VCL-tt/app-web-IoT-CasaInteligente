...
export class PasilloComponent {
  ...
  toggleLed(ledKey: string, currentState: number | undefined, pasilloNumero: number): void {
    const newState = currentState === 1 ? 0 : 1;
    const mensaje = `Se ${newState === 1 ? 'encendió' : 'apagó'} la luz del Pasillo ${pasilloNumero}`;
